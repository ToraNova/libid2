...
int sockconn(int sockobj, const char *srvaddr,int portnum){
	...
	debug("Connection to %s on port %d returns %d",srvaddr,portnum,retval);
	return retval;
}
...
int sockbind(int sockobj, int portnum, int reuse_bool){
	...
	debug("Socket bound to port %d returns %d",portnum,retval);
	return retval;
}
...
int sendbuf(int sockobj, char *sendbuffer, size_t buflen, int timeout_sec){
	...
	//debug("Buffer %s of size %zu sent with retval:%d",sendbuffer,buflen,sent);
	debug("Buffer of size %zu sent with retval:%d",buflen,sent);
	return sent;
}
...
int recvbuf(int sockobj, char *recvbuffer, size_t buflen, int timeout_sec){
	...
	//debug("Buffer %s of size %zu recv with retval:%d",recvbuffer,buflen,retval);
	debug("Buffer of size %zu recv with retval:%d",buflen,retval);
	return retval;
}
...
int fixed_recvbuf(int sockobj, char *recvbuffer, size_t buflen, int timeout_sec){
	...
	//debug("Buffer %s of size %zu recv with retval:%d %d",recvbuffer,buflen,brecv,ptrindex);
	debug("Buffer of size %zu recv with retval:%d %d",buflen,brecv,ptrindex);
