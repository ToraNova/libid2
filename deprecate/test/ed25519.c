...
//#define str_messagefile "res/c25519/message"
#define str_messagefile "res/c25519/id"
#define str_signfile 	"res/c25519/signature"
...
int main(int argc, char *argv[]){
	...
	if(argc > 1){
		...
		if( strcmp(argv[1], "gen") == 0 ){
			...
			rc = e25519_keygen( &pbuf, &plen, &sbuf, &slen);
			if(rc != 0)log_err("Keygen Error");
			...
			rc = e25519_sign(sbuf, slen, mbuf, mlen, &obuf, &olen);
			if(rc != 0)log_err("Sign Error");
			...
			rc = e25519_verify( pbuf, plen, mbuf, mlen, obuf, olen);
			if(rc==0){
